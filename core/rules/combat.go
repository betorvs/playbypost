...
type Attack struct {
	...
	Weapon   string
	Kind     AttackTypes
	Attacker RolePlaying
	Defensor RolePlaying
...
func (a *Attack) singleMeleeAttack() {
	rpgSystem := a.Attacker.RPGSystem()
	a.Logger.Info("single melee attack call", "system_name", rpgSystem.Name)
	strenght := "strenght"
	...
	case rpg.D10HM:
		...
		attacker := a.Attacker.Attack(weaponry, a.Weapon)
		defense := a.Defensor.DefenseBonus("melee")
		calcDices := a.Dice.FormatDice(attacker-defense, 0)
		a.Logger.Info("calcDices", "dices", calcDices)
		result, _ := a.Dice.FreeRoll(diceName, calcDices)
		a.Logger.Info("dice result", "result", attacker-defense, "rolled", result.Rolled)
		a.Response.Text = result.Rolled
		if result.Result > 0 {
			a.Response.Success = true
			a.Response.Damage = result.Result
			err := a.Defensor.Damage(result.Result)
			if err != nil {
				a.Logger.Error("error on damage calc", "error", err)
			}
			if a.Defensor.HealthStatus() <= 0 {
				_ = a.Defensor.Destroy()
			}
		}
		a.Logger.Info("results", "round", a.Round, "details", a.Response.Text)
...
func NewAttack(round, weapon string, kind AttackTypes, attacker RolePlaying, defensor RolePlaying, dice rpg.RollInterface, logger *slog.Logger) *Attack {
	a := Attack{
		Round:    round,
		Weapon:   weapon,
		Kind:     kind,
