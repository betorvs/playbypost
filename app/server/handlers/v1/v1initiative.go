...
func (a MainApi) GenerateInitiative(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("%+v \n", obj)
	enc, err := a.db.GetEncounterByID(a.ctx, obj.EncounterID)
	if err != nil {
		a.s.ErrJSON(w, http.StatusBadRequest, "encounter not found")
		return
	}
	var phase types.Phase
	if phase == types.Started {
		party := make(map[string]int)
		players, err := a.db.GetPlayersByEncounterID(a.ctx, obj.EncounterID, obj.NPC, a.rpg)
		if err != nil {
	...
	a.s.ErrJSON(w, http.StatusBadRequest, "encounter not in start phase")
}
